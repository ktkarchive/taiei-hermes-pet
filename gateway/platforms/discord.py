...
def _build_allowed_mentions():
    ...
    allowed = allowed_mentions_cls(
        **values,
    )

    # Some tests and lightweight gateway installs use a mocked ``discord``
    # module. MagicMock constructors accept the kwargs above but return a mock
    # whose attributes are still MagicMocks, not booleans. Force the observable
    # flags onto the object so policy checks remain deterministic.
    for name, value in values.items():
        try:
            setattr(allowed, name, value)
        except Exception:
            pass
    return allowed
        ...
        def __init__(
            ...
            for name in (
                ...
                object.__setattr__(
                    self,
                    name,
                    getattr(type(self), name).__get__(self, type(self)),
                )
