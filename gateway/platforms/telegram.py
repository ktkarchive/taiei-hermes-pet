...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _build_message_event(
        ...
        def _chat_type_matches(value: Any, *names: str) -> bool:
            raw_values = [value]
            for attr in ("value", "name"):
                try:
                    raw_values.append(getattr(value, attr))
                except Exception:
                    pass
            normalized = {str(v).strip().lower() for v in raw_values if v is not None}
            for name in names:
                expected = name.strip().lower()
                if expected in normalized:
                    return True
                if any(item.endswith(f".{expected}") for item in normalized):
                    return True
            return False

        # Determine chat type. Tests and optional installs can leave ChatType
        # backed by a mock object, so also accept Telegram's raw string values
        # and the negative-id convention used by groups/supergroups.
        chat_type = "dm"
        raw_chat_type = getattr(chat, "type", None)
        if (
            raw_chat_type in (ChatType.GROUP, ChatType.SUPERGROUP)
            or _chat_type_matches(raw_chat_type, "group", "supergroup")
        ):
            chat_type = "group"
        elif raw_chat_type == ChatType.CHANNEL or _chat_type_matches(raw_chat_type, "channel"):
            chat_type = "channel"
        elif _chat_type_matches(raw_chat_type, "private"):
            chat_type = "dm"
        else:
            try:
                if int(chat.id) < 0:
