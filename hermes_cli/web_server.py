...
from hermes_cli.pet_forwarder import pet_event_from_gateway_frame
from hermes_pet.protocol import (
    PET_RELAY_HEADER_NAME as _PET_RELAY_HEADER_NAME,
    PET_RELAY_TOKEN_ENV as _PET_RELAY_TOKEN_ENV,
    PET_STATES as _PET_RELAY_STATES,
    PROTOCOL_VERSION as _PET_PROTOCOL_VERSION,
    RUNTIME_FILENAME as _PET_OVERLAY_RUNTIME_FILENAME,
    clean_source_id as _clean_pet_source_id,
    clean_text as _clean_pet_text,
)
...
_PET_OVERLAY_FORWARD_INTERVAL_SECONDS = 0.35
...
def _configured_pet_relay_token() -> str:
    return os.getenv(_PET_RELAY_TOKEN_ENV, "").strip()


def _has_valid_pet_relay_token(request: Request) -> bool:
    """Validate either the dashboard session token or the opt-in relay token."""
    relay_token = _configured_pet_relay_token()
    relay_header = request.headers.get(_PET_RELAY_HEADER_NAME, "")

    if relay_token and relay_header and hmac.compare_digest(
        relay_header.encode(),
        relay_token.encode(),
    ):
        return True

    # Dashboard-originated test posts may use the normal ephemeral token.
    return _has_valid_session_token(request)


def _require_pet_relay_token(request: Request) -> None:
    if not _has_valid_pet_relay_token(request):
        raise HTTPException(status_code=401, detail="Unauthorized")
...
class PetRelayEvent(BaseModel):
    """Small cross-machine status pulse for the dashboard pet renderer."""
...
_pet_relay_subscribers: set[WebSocket] = set()
_pet_relay_lock = asyncio.Lock()
_pet_overlay_last_state: str | None = None
_pet_overlay_last_forward_at = 0.0
...
async def _broadcast_event(channel: str, payload: str) -> None:
    ...
    _queue_pet_overlay_forward(payload)
...
def _pet_overlay_runtime_path() -> Path:
    return get_hermes_home() / "runtime" / _PET_OVERLAY_RUNTIME_FILENAME


def _read_pet_overlay_runtime() -> Optional[dict[str, Any]]:
    try:
        data = json.loads(_pet_overlay_runtime_path().read_text())
    except Exception:
        return None
...
def _local_pet_event_from_frame(frame_text: str) -> Optional[dict[str, Any]]:
    return pet_event_from_gateway_frame(frame_text)


def _queue_pet_overlay_forward(frame_text: str) -> None:
    global _pet_overlay_last_forward_at, _pet_overlay_last_state

    event = _local_pet_event_from_frame(frame_text)
    if not event:
        return
    ...
    if (
        state == _pet_overlay_last_state
        and now - _pet_overlay_last_forward_at < _PET_OVERLAY_FORWARD_INTERVAL_SECONDS
    ):
        return

    runtime = _read_pet_overlay_runtime()
    if not runtime:
        return

    _pet_overlay_last_state = state
    _pet_overlay_last_forward_at = now
    asyncio.create_task(
        _post_pet_overlay_event(str(runtime["url"]), str(runtime["token"]), event)
    )


async def _post_pet_overlay_event(
    url: str,
    token: str,
    event: dict[str, Any],
) -> None:
...
def _normalize_pet_relay_event(body: PetRelayEvent) -> dict[str, Any]:
    state = str(body.state or "idle").strip().lower()
    if state not in _PET_RELAY_STATES:
        raise HTTPException(
            status_code=400,
            detail=f"state must be one of: {', '.join(sorted(_PET_RELAY_STATES))}",
        )

    ttl_ms = body.ttl_ms
    if ttl_ms is not None:
        ttl_ms = max(1_000, min(int(ttl_ms), 600_000))

    payload: dict[str, Any] = {
        "protocol": _PET_PROTOCOL_VERSION,
        "source_id": _clean_pet_source_id(body.source_id),
        "label": _clean_pet_text(body.label, "Remote Hermes", 64),
        "state": state,
        "message": (
            _clean_pet_text(body.message, "", 180)
            if body.message is not None
            else None
        ),
        "event_type": (
            _clean_pet_text(body.event_type, "", 64)
            if body.event_type is not None
            else None
        ),
        "severity": (
            _clean_pet_text(body.severity, "", 32)
...
async def _broadcast_pet_relay(payload: dict[str, Any]) -> int:
    """Fan out a remote pet event to every dashboard subscriber."""
    frame = json.dumps(
        {"type": "pet.relay", "payload": payload},
        ensure_ascii=False,
        separators=(",", ":"),
    )
    async with _pet_relay_lock:
        subs = list(_pet_relay_subscribers)
...
@app.get("/api/pet/relay/status")
async def pet_relay_status():
    async with _pet_relay_lock:
        subscriber_count = len(_pet_relay_subscribers)

    return {
        "relay_enabled": bool(_configured_pet_relay_token()),
        "token_header": _PET_RELAY_HEADER_NAME,
        "token_env": _PET_RELAY_TOKEN_ENV,
        "subscriber_count": subscriber_count,
        "states": sorted(_PET_RELAY_STATES),
    }


@app.post("/api/pet/relay/events")
async def pet_relay_event(body: PetRelayEvent, request: Request):
    """Receive one remote Hermes status pulse and fan it out to pet clients.

    Remote writers must set HERMES_PET_RELAY_TOKEN on this dashboard process
    and send it in X-Hermes-Pet-Relay-Token. The local dashboard may also post
    with the ephemeral session token, which keeps smoke testing simple.
    """
    _require_pet_relay_token(request)
    payload = _normalize_pet_relay_event(body)
    delivered = await _broadcast_pet_relay(payload)
    return {"ok": True, "delivered": delivered, "event": payload}


@app.websocket("/api/pet/relay/events")
async def pet_relay_events_ws(ws: WebSocket) -> None:
    token = ws.query_params.get("token", "")
    if not hmac.compare_digest(token.encode(), _SESSION_TOKEN.encode()):
        await ws.close(code=4401)
        return
    ...
    async with _pet_relay_lock:
        _pet_relay_subscribers.add(ws)

    try:
        await ws.send_text(
            json.dumps(
                {
                    "type": "pet.relay.ready",
                    "payload": {
                        "relay_enabled": bool(_configured_pet_relay_token()),
                        "states": sorted(_PET_RELAY_STATES),
                    },
        ...
        async with _pet_relay_lock:
            _pet_relay_subscribers.discard(ws)
