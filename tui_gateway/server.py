...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    try:
        heartbeat = session.get("pet_session_heartbeat")
...
def _emit(event: str, sid: str, payload: dict | None = None):
    try:
        ...
        forward_gateway_event(event, payload)
    except Exception:
        pass
...
def _apply_pending_title(session: dict, key: str, *, model: str | None = None) -> None:
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # pending_title is retried here if the UI queued a title while the
            # agent was still building; later message completion keeps a second
            # best-effort retry for rows created by run_conversation().
            current["agent"] = agent
            _apply_pending_title(current, key, model=getattr(agent, "model", _resolve_model()))
...
def _pet_session_label(session: dict) -> str:
    ...
    if title:
        return str(title)
    suffix = session_key[-12:]
    return f"Hermes TUI {suffix}" if suffix else "Hermes TUI"


def _attach_pet_session_heartbeat(session: dict) -> None:
    if session.get("pet_session_heartbeat") is not None:
        return
    try:
        from hermes_cli.pet_sessions import start_pet_session_heartbeat

        session["pet_session_heartbeat"] = start_pet_session_heartbeat(
            mode="tui",
            get_session_id=lambda _session=session: str(_session.get("session_key") or ""),
            label=lambda _session=session: _pet_session_label(_session),
            cwd=os.getenv("TERMINAL_CWD", os.getcwd()),
            command="hermes --tui",
        )
    except Exception:
        session["pet_session_heartbeat"] = None
...
def _init_session(sid: str, key: str, agent, history: list, cols: int = 80):
    ...
    _attach_pet_session_heartbeat(session)
    try:
