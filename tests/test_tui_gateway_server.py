...
def test_session_create_attaches_pet_heartbeat_to_lazy_session(monkeypatch):
    attached: list[dict] = []
    ...
    try:
        assert attached == [server._sessions[sid]]
        assert attached[0]["session_key"]
    finally:
        server._sessions.pop(sid, None)
