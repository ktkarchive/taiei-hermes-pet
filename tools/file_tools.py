...
def _is_allowed_temp_path(path: str) -> bool:
    candidates = {tempfile.gettempdir(), "/tmp", "/private/tmp"}
    for candidate in candidates:
        try:
            real_candidate = os.path.realpath(candidate)
        except OSError:
            real_candidate = candidate
        if _is_under_path(path, real_candidate) or _is_under_path(path, candidate):
            return True
    return False
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if _is_allowed_temp_path(real_resolved) or _is_allowed_temp_path(normalized):
        return None
    for prefix in _SENSITIVE_PATH_PREFIXES:
        if real_resolved.startswith(prefix) or resolved.startswith(prefix) or normalized.startswith(prefix):
            return _err
