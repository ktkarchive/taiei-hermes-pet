...
def _pet_forward(event_type: str, payload: Any = None) -> None:
    try:
        from hermes_cli.pet_forwarder import forward_gateway_event

        forward_gateway_event(event_type, payload)
    except Exception:
        pass
...
class HermesCLI:
    ...
    def _on_thinking(self, text: str) -> None:
        ...
        if text:
            _pet_forward("thinking.delta", {"message": text[:180]})
        if not text:
    ...
    def _stream_delta(self, text) -> None:
        ...
        _pet_forward("message.delta", {"text": str(text)[:180]})
    ...
    def _on_tool_gen_start(self, tool_name: str) -> None:
        ...
        """
        _pet_forward("tool.generating", {"name": tool_name})
        if getattr(self, "_stream_box_opened", False):
            self._flush_stream()
            self._stream_box_opened = False
        self._close_reasoning_box()
        ...
        if event_type == "tool.completed":
            _pet_forward(
                "tool.complete",
                {
                    ...
                    "preview": preview or function_name or "tool complete",
    ...
    def _clarify_callback(self, question, choices):
        ...
        _pet_forward("clarify.request", {"context": "clarify requested"})
    ...
    def _sudo_password_callback(self) -> str:
        ...
        _pet_forward("sudo.request", {"context": "sudo password requested"})
        timeout = 45
        response_queue = queue.Queue()
    ...
    def _approval_callback(self, command: str, description: str,
        ...
        _pet_forward("approval.request", {"context": "approval needed"})
    ...
    def _secret_capture_callback(self, var_name: str, prompt: str, metadata=None) -> dict:
        _pet_forward("secret.request", {"context": "secret requested", "name": var_name})
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        try:
            ...
            result = None
            _pet_forward("message.start", {"message": "working"})
            ...
            if result is None:
                _pet_forward("message.error", {"error": "run did not return a result"})
            else:
                _pet_forward(
                    "message.complete",
                    {
                        ...
                        "summary": (response or result.get("error") or "ready")[:180],
                    },
                )
            ...
            _pet_forward("message.error", {"error": str(e)[:180]})
            print(f"Error: {e}")
            return None
        finally:
    ...
    def _pet_session_label(self) -> str:
        title = None
        try:
        ...
        if title:
            return str(title)
        suffix = str(self.session_id or "")[-12:]
        return f"Hermes CLI {suffix}" if suffix else "Hermes CLI"
    ...
    def run(self):
        ...
        self._pet_session_heartbeat = None
        try:
            from hermes_cli.pet_sessions import start_pet_session_heartbeat

            self._pet_session_heartbeat = start_pet_session_heartbeat(
                mode="cli",
                get_session_id=lambda: self.session_id,
                label=self._pet_session_label,
                cwd=os.getcwd(),
                command="hermes",
            )
        except Exception:
        ...
        try:
            ...
            if self.agent and getattr(self, '_agent_running', False):
                try:
                    ...
                    pass
            if self._pet_session_heartbeat is not None:
                try:
                    self._pet_session_heartbeat.stop()
                except Exception:
                    pass
            _run_cleanup()
            self._print_exit_summary()
