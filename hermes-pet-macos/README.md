...
Hermes Pet is a macOS desktop pet for Hermes Agent. The public MVP installs a
removable `hermes-pet` command and Hermes skill wrapper, without changing the
user's global `hermes` command or upstream Hermes checkout.
...
Or install from the Hermes package directory:
